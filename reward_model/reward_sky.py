...
"""
MODULE TO IMPLEMENT FINETUNED MODEL + SKYWORK REWARD MODEL
"""
...
3. 
Load the Skywork reward model (Skywork/Skywork-Reward-V2-Qwen3-0.6B) and its specific tokenizer.
...
def load_models():
    ...
    # 3. Load the Skywork reward model and its specific tokenizer
    print("Loading Skywork reward model and tokenizer...")
...
def calculate_reward(reward_inputs, reward_model, reward_tokenizer, device):
    """
    Calculates the reward score for given inputs using the Skywork reward model.

    Args:
        reward_inputs (list): A list of strings, each being an input for the reward model.
        reward_model: The Skywork reward model.
...
def aggregate_report(processed_data):
    ...
    print("\n--- Skywork Reward Model Evaluation Report ---")
    ...
    ax1.set_title('Comparison of Reward Distributions (Skywork Model)')
    ...
    ax2.set_title('Side-by-Side Box Plot of Rewards (Skywork Model)')
    ax2.set_ylabel('Reward Value')
    ax2.yaxis.grid(True) # Add horizontal grid lines for better readability

    plt.suptitle('Human vs. Finetuned Model Reward Comparison (Skywork Model)', fontsize=16)
    ...
    for i, item in enumerate(processed_data[:20]):
        ...
        print(f"  -> Skywork Reward: {item['human_reward']:.4f}")
        print("-" * 20)
        print(f"Base Model Summary: {item['base_summary']}")
        print(f"  -> Skywork Reward: {item['base_reward']:.4f}")
        print("-" * 20)
        print(f"Fine-tuned Summary: {item['finetuned_summary']}")
        print(f"  -> Skywork Reward: {item['finetuned_reward']:.4f}")


def main():
    """Main execution function."""
    print("Starting Skywork reward model evaluation...")
