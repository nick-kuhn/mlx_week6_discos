        ...
        if checkpoint_file:
            ...
            if "lora_state_dict" in checkpoint:
                ...
                lora_config = LoraConfig(
                    r=lora_settings.get(
                        "r", 4
                    ),  # Default to 4 to match finetune_default.yaml
                    lora_alpha=lora_settings.get(
                        "alpha", 8
                    ),  # Default to 8 to match finetune_default.yaml
                    target_modules=lora_settings.get(
                ...
                print(
                    f"Creating LoRA model with r={lora_config.r}, alpha={lora_config.lora_alpha}"
                )
...
def main():
    ...
    for epoch in range(ppo_config.num_ppo_epochs):
        for batch in tqdm(ppo_trainer.dataloader):
            ...
            print(f"Step {epoch}: Mean Reward = {stats['ppo/returns/mean']:.2f}")
